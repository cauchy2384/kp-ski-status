 ...
 */
function _parseHtmlValue(input) {
    let result = "";
    const regExp = /"(.*?)"/;
    ...
    if (has.call(tablesAsJson, slopesTableNumber)) {
        ...
        Object.values(slopesTable).forEach((slopeRow) => {
            if (has.call(slopeRow, '0') && has.call(slopeRow, '_5')) {
                ...
                let status = _parseHtmlValue(slopeRow['_5']);
                switch (status) {
                    case 'green': status = 'opened'; break;
                    case 'yellow':
                    case 'red': status = 'closed'; break;
    ...
    liftsTable.find('tr').each((index, element) => {
        ...
        $(element).find('td').each((i, e) => {
            if (i === 0) {
                ...
                switch (status) {
                    case 'Подъёмник открыт': status = 'opened'; break;
                    case 'Подъёмник временно закрыт':
                    case 'Подъёмник закрыт': status = 'closed'; break;
                    default: status = 'unknown'; break;
                }
            }
        });
...
async function _getWeather() {
    ...
    const $ = cheerio.load(page);
    const table = $('table[class=\"wthr_struct _main\"]');
    table.find('tr > td').each((index, element) => {
        ...
        $(element).find('div').each((i, e) => {
            switch (i) {
                case 0: name = $(e).text(); break;
                case 1: temperature = $(e).text(); break;
                case 2: wind = $(e).text(); break;
                case 3: snow = $(e).text(); break;
                default: break;
            }
        });
        if (name) {
...
async function get() {
    let result = {};
    try {
        const slopes = await _getSlopes();
        const lifts = await  _getLifts();
        const weather = await _getWeather();
